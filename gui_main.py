...
import sys, os
import numpy as np
from datasetm.mnist import load_mnist
...
from hanzi import get_label_dict, inference
...
MODE_MNIST = 1             
MODE_WRITE = 2             
MODE_LSTM_HANZIAUTO = 3    
MODE_LSTM_HANZIWRITE = 4   
...
class MainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self):
        ...
        self.model.load_state_dict(torch.load(crnn_model_path,map_location='cpu'))
    ...
    def cbBox_Mode_Callback(self, text):
        ...
        elif text == '3：汉字手写输入法1':
            self.mode = MODE_LSTM_HANZIAUTO
            self.clearDataArea()
            self.pbtGetMnist.setEnabled(False)
        ...
        elif text == '4：汉字手写输入法2':
            self.mode = MODE_LSTM_HANZIWRITE
            self.clearDataArea()
            self.pbtGetMnist.setEnabled(False)
    ...
    def crnn_single_test(self,cropped_image, model):
        ...
        self.lbResult.setText("%s" % (sim_pred))
    ...
    def hanzi1(self,img):
        pil_img = ImageQt.fromqimage(img)
        pil_img = pil_img.resize((64, 64), Image.ANTIALIAS).convert('RGB')
        
        pil_img.save('tmphanzi.png')

        final_predict_val, final_predict_index = inference('tmphanzi.png')
        # 获取index对应的文字
        label_dict = get_label_dict()
        
        self.result[0] = label_dict[final_predict_index[0][0]]  # 预测的数字
        self.result[1] = final_predict_val[0][0]     # 置信度
        
        self.lbResult.setText("%s" % (self.result[0]))
        self.lbCofidence.setText("%.8f" % (self.result[1]))
    ...
    def hanzi2(self,img):
        ...
        pil_img.save('tmphanzi.png')
        image = Image.open('tmphanzi.png')
        self.crnn_single_test(image, self.model)
         
    
    # 识别
    def pbtPredict_Callback(self):
        __img =[]      # 将图像统一从qimage->pil image -> np.array [1, 1, 28, 28]
        ...
        elif self.mode == MODE_LSTM_HANZIWRITE:
            __img = self.paintBoard.getContentAsQImage()
            self.hanzi1(__img)
        
        elif self.mode == MODE_LSTM_HANZIAUTO:
            __img = self.paintBoard.getContentAsQImage()
            self.hanzi2(__img)
